...
to enumerate-turtles ;;

  ask turtles [
    find-admissable-flockmates
    set flock-index who
    set avg-dir heading
  ]

  let flock-names []
  let flock-names-used []

  ask turtles [

    set-cluster-flockmates

  ]

  ask turtles [

    set flock-names insert-item 0 flock-names flock-index

  ]

  set flock-names remove-duplicates flock-names

  ;task: to split into functions + optimize for lookups
  ;compute means flock wise
  ask turtles [

    ;;ifelse not member? flock-index flock-names-used [
    set current flock-index
    compute-avg-dir
    ;;set flock-names-used insert-item flock-index flock-names-used flock-index
    ;;]
    ;;[
      ;;set avg-dir item flock-index flock-names-used
    ;;]

  ]

  ;compute varinces flock wise
  ask turtles [
  ...
  ;pretty display labeled data
  ask turtles [

    set label word (word (word (word flock-index " | ") round avg-dir) " | ") round var-dir

  ]

  ;set flock-count count get-unique-flocks

end

to-report get-num-flocks
  ;reports the number of unique flock identifiers
  report length (remove-duplicates ([flock-index] of turtles))
end

to-report get-mean-dir
  report mean (remove-duplicates ([avg-dir] of turtles))
end

to-report get-mean-var
  report mean (remove-duplicates([sqrt var-dir] of turtles))
end

to compute-var-dir
  set var-dir mean [heading ^ 2] of turtles with [flock-index = current] - avg-dir ^ 2
end

to compute-avg-dir
  set avg-dir mean [heading] of turtles with [flock-index = current]
end

to set-cluster-nearest
  set flock-index min [flock-index] of admissable-flockmates
end

to set-cluster-flockmates
  set flock-index min [flock-index] of admissable-flockmates
end
...
to find-admissable-flockmates
  let u_bound 2 * minimum-separation
  set admissable-flockmates turtles in-radius u_bound
end
...
<experiments>
  <experiment name="experiment" repetitions="30" runMetricsEveryStep="true">
    ...
    <go>go</go>
    <timeLimit steps="350"/>
    <metric>get-num-flocks</metric>
    <metric>get-mean-dir</metric>
    <metric>get-mean-var</metric>
